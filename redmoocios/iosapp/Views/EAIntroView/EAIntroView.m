...
//    self.pageControl.numberOfPages = _pages.count;  //暂时去除  引导页图片中包含了点
    [self addSubview:self.pageControl];
    
    self.skipButton = [[UIButton alloc] initWithFrame:CGRectMake(self.scrollView.frame.size.width - 80, 30, 60, self.pageControl.frame.size.height)];
    [self.skipButton.titleLabel setFont: [UIFont systemFontOfSize: 12.0]];
    //[self.skipButton setBackgroundColor:kNBR_ProjectColor];
     [self.skipButton setBackgroundColor:[[UIColor blackColor] colorWithAlphaComponent:0.3]];
    //self.skipButton .contentEdgeInsets = UIEdgeInsetsMake(5,0, 5, 0);
