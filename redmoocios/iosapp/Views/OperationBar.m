...
- (void)setLayout
{
    ...
    NSArray *images    = @[@"toolbar-keyboardUp", @"toolbar-separatorline", @"toolbar-comments", @"toolbar-editingComment", @"toolbar-star", @"toolbar-zan", @"toolbar-share1"];
    NSArray *selectors = @[@"switchMode:", @"", @"showComments:", @"editComment:", @"toggleStar:", @"zan:", @"share:"];
    ...
    [images enumerateObjectsUsingBlock:^(NSString *imageName, NSUInteger idx, BOOL *stop) {
        ...
        if (idx != 1) {
            ...
            [button setEnlargedEdgeWithTop:10 left:15 bottom:10 right:15];
...
- (void)setIsStarred:(BOOL)isStarred
{
    ...
    if (isStarred) {
        [starButton setImage:[UIImage imageNamed:@"toolbar-star-selected"] forState:UIControlStateNormal];
    } else {
...
{
    UIBarButtonItem *zanBarButton = self.items[10];
    UIButton *zanButton = (UIButton *)zanBarButton.customView;
    
    if (isZan) {
        [zanButton setImage:[UIImage imageNamed:@"toolbar-zan-selected"] forState:UIControlStateNormal];
    } else {
        [zanButton setImage:[UIImage imageNamed:@"toolbar-zan"] forState:UIControlStateNormal];
    }
