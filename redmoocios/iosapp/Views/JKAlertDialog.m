...
- (instancetype)initWithTitle:(NSString *)title
                      attMessage:(NSMutableAttributedString *)message{
    self = [super init];
    if (self) {
        _items = [[NSMutableArray alloc] init];
        _title  = title;
...
-(void)buildViews{
    ...
    //message
    CGFloat messageHeigh;
    if ([_message  isEqual: @""]) {
        messageHeigh = [self heightWithString:[_attMessage string] fontSize:14 width:AlertWidth-2*AlertPadding];
        ...
        _labelmessage.attributedText = _attMessage;
    }else {
        messageHeigh = [self heightWithString:_message fontSize:14 width:AlertWidth-2*AlertPadding];
        _labelmessage =  [[UILabel alloc]initWithFrame:CGRectMake(AlertPadding, _labelTitle.frame.origin.y+_labelTitle.frame.size.height, AlertWidth-2*AlertPadding, messageHeigh+2*AlertPadding)];
