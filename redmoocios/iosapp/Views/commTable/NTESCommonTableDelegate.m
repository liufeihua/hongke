...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tableRow.showSwitch) {
        UISwitch *switchview = [[UISwitch alloc] initWithFrame:CGRectZero];
        switchview.on = tableRow.switchValue;
        UIViewController *vc = tableView.viewController;
        NSString *switchActionName = tableRow.switchActionName;
        
        if (switchActionName.length) {
            SEL sel = NSSelectorFromString(switchActionName);
            [switchview addTarget:vc action:sel forControlEvents:UIControlEventValueChanged];
        }
...
- (CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section{
    ...
    NTESCommonTableSection *tableSection = self.data[section];
    CGSize size = [tableSection.headerTitle sizeWithAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:13.f]}];
    return size.height;
}
...
- (void)refreshData:(NTESCommonTableRow *)rowData cell:(UITableViewCell *)cell{
    //cell.imageView.image = [UIImage imageNamed:@"icon_blacklist_normal"];
