...
- (void)setCurrentIndexPath:(NSIndexPath *)currentIndexPath {
    ...
    NSInteger row =  [self caculateIndex:currentIndexPath.row];
    if(self.customPageControl == nil)
        self.pageControl.currentPage = row;
    else
        self.customPageControl.currentPage = row;
//    NSLog(@"row=%ld,subviews=%ld",row,_carouselView.subviews.count);
//    if (_carouselView.subviews.count > row) {
//
//        UICollectionViewCell *nowCell = _carouselView.subviews[row];
//        UIImageView *nowImgView = (UIImageView *)nowCell.contentView.subviews[0];
//        nowImgView.alpha = 1;
//    }
}
...
//        NSString *str = @"H:|-0-[view]-0-|";
        NSString *str;
        if (self.style == CWCarouselStyle_H_3){
            str = @"H:|-20-[view]-20-|";
        }else{
            str = @"H:|-0-[view]-0-|";
        }
