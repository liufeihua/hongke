...
- (void)playWithModel:(TracksViewModel *)tracks indexPathRow:(NSInteger ) indexPathRow{
    ...
    _tracksVM = tracks;
    _rowNumber = tracks.radios.totalCount;
    _indexPathRow = indexPathRow;

    //缓存播放实现，可自行查找AVAssetResourceLoader资料,或采用AudioQueue实现
    NSURL *musicURL = [NSURL URLWithString:self.tracksVM.radios.list[_indexPathRow].audio];
...
- (void)playPreviousMusic{
    ...
    if (_currentPlayerItem){
        self.tracksVM.radios.list[_indexPathRow].isPlaying = [NSNumber numberWithInt:0];
        if (_indexPathRow > 0) {
            _indexPathRow--;
        }else{
            _indexPathRow = _rowNumber-1;
        }
        self.tracksVM.radios.list[_indexPathRow].isPlaying = [NSNumber numberWithInt:1];
        NSURL *musicURL = [NSURL URLWithString:self.tracksVM.radios.list[_indexPathRow].audio];
        _currentPlayerItem = [AVPlayerItem playerItemWithURL:musicURL];
...
- (NSString *)playMusicName{
    return self.tracksVM.radios.list[_indexPathRow].title;
}

- (int)playArticleID{
    return [self.tracksVM.radios.list[_indexPathRow].articleId intValue];
}

- (NSString *)playSinger{

    return self.tracksVM.radios.list[_indexPathRow].author;

}

- (NSString *)playMusicTitle{
    return self.tracksVM.cateName;

}

- (NSURL *)playCoverLarge{

    return self.tracksVM.radios.list[_indexPathRow].image;
}
...
- (void) setFavoriteMusic:(int) collected{
    self.tracksVM.radios.list[_indexPathRow].collected = [NSNumber numberWithInt:collected];
}

- (int)hasBeenFavoriteMusic{
    return [self.tracksVM.radios.list[_indexPathRow].collected intValue];
}
