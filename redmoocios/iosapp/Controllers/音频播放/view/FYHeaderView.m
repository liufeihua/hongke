...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        if(![_visualEffectView isDescendantOfView:self]) {
            ...
            _visualEffectView.frame = CGRectMake(0, 0, kNBR_SCREEN_W, frame.size.height+20);
            [self addSubview:_visualEffectView];
        }

        [self.topLeftBtn setImage:[UIImage imageNamed:@"nav_back"] forState:UIControlStateNormal];
...
- (void)setVisualEffectFrame:(CGRect)visualEffectFrame{
    ...
    _visualEffectView.frame = CGRectMake(0, 0, kNBR_SCREEN_W, height);
}
