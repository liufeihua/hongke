...
-(void)confingSubViews{
    ...
    self.deleteButton = [[UIButton alloc] initWithFrame:CGRectMake(0, -2, 15, 15)];
    [self.deleteButton setBackgroundImage:[UIImage imageNamed:@"columnDelete"] forState:UIControlStateNormal];
...
-(void)configCell:(NSArray *)dataArray withIndexPath:(NSIndexPath *)indexPath{
    ...
    GFKDUserNode *nodeModel = [[GFKDUserNode alloc] initWithDict:dataArray[indexPath.row]];
    self.contentLabel.text = nodeModel.cateName;
